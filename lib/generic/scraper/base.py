...
class Scraper:
    ...
    def authenticate(self, username: str, password: str) -> User:
        ...
        if not user or not self._server.is_authenticated(user):
            user = self._server.authenticate_by_password(username, password)
            _user_cache[user.user] = user
        _log.debug('authenticate: user=%s', user)
        return user
    ...
    def _get_items(self, user: User, item_type: str, media_type: str,
        ...
        jf_items, _, _ = self._server.get_items(user, params=params)
