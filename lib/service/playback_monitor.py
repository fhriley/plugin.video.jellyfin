...
class PlaybackMonitor(xbmc.Player):
    ...
    def getTime(self) -> float:
        try:
            ...
            self._log_holder.log.exception('getTime failed')
        return self._latest_time

    def onPlayBackStarted(self):
        self._log_holder.log.debug('onPlayBackStarted()')
        try:
            self._playing_state = PlayingState(self.getTime(), self.getPlayingItem())
            if self._playing_state.jf_id:
                self._log_holder.log.debug('playing jf_id=%s kodi_id=%s', self._playing_state.jf_id,
                                           self._playing_state.kodi_id)
                self._server.send_playback_started(self._user, self._playing_state.jf_id, self._playing_state.start_s)
            else:
                self._log_holder.log.warning('no jellyfin id for item id %s', self._playing_state.kodi_id)
        except Exception:
            self._log_holder.log.exception('onPlayBackStarted failed')

    def onPlayBackEnded(self):
        self._log_holder.log.debug('onPlayBackEnded()')
        self.onPlayBackStopped()

    def onPlayBackStopped(self):
        self._log_holder.log.debug('onPlayBackStopped()')
        try:
            if self._playing_state and self._playing_state.jf_id:
                if self._playing_state.media_type == 'episode':
                    details = get_kodi_episode_details(self._log_holder.log, self._playing_state.kodi_id, 'playcount',
                                                       'resume')
                else:
                    details = get_kodi_movie_details(self._log_holder.log, self._playing_state.kodi_id, 'playcount',
                                                     'resume')
            ...
            self._log_holder.log.exception('onPlayBackStopped failed')

    def onPlayBackError(self):
        self._log_holder.log.debug('onPlayBackError()')
        self.onPlayBackStopped()

    def onPlayBackPaused(self):
        self._log_holder.log.debug('onPlayBackPaused()')
        try:
            if self._playing_state and self._playing_state.jf_id:
                self._server.send_playback_paused(self._user, self._playing_state.jf_id, self.getTime())
        except Exception:
            self._log_holder.log.exception('onPlayBackPaused failed')

    def onPlayBackResumed(self):
        self._log_holder.log.debug('onPlayBackResumed()')
        try:
            if self._playing_state and self._playing_state.jf_id:
                self._server.send_playback_unpaused(self._user, self._playing_state.jf_id, self.getTime())
        except Exception:
            self._log_holder.log.exception('onPlayBackResumed failed')

    def onPlayBackSeek(self, time, seekOffset):
        time /= 1000.0
        seekOffset /= 1000.0
        self._log_holder.log.debug('onPlayBackSeek(%s, %s)', time, seekOffset)
        try:
            self._latest_time = time
            if self._playing_state and self._playing_state.jf_id:
                self._server.send_playback_time(self._user, self._playing_state.jf_id, time)
        except Exception:
            self._log_holder.log.exception('onPlayBackSeek failed')
